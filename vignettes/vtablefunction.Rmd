...
`data.title` and `desc` will only show up in full-page `vtable`s. That is, you won't get them with `out = 'return'`, or `out = 'latex'` (although `out = 'latexpage'` works). `note` and `anchor` will only show up in formats that support multi-column cells and anchoring, so they won't work with `out = 'return'`.
